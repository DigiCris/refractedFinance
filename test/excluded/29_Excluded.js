...
contract('No Excluidas', accounts => {
    ...
    context("29- Tx entre dos dir. excluidas, ambas con astkfee y devfee 0%", async () => {
        ...
        it("23 - transfer between excluded", async ()=> {
            try{
                await erc20_instance.transfer(std1, '7970'+weiToEth, { from: std2 });
                expect(1).to.equal(1);
            } catch(error){
                expect(error).to.equal("no debio entrar aca");
            }
