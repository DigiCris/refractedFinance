...
contract('No Excluidas', accounts => {
    ...
    context("20- Tx entre una dir. excluida y otra no excluida, ambas con astkfee en 0% y una con devfee en 0% y la otra con devfee positivo.", async () => {
        ...
        it("09 - Verify balances", async ()=> {
            await checkBalance(owner,'19990000');
            await checkBalance(std1,'9975');
            await checkBalance(dev,'25');
        });


        it("10 - setAStkFee to 0 to std2 (-1)", async ()=> {
            await erc20_instance.setAStkFee(std2, false, '-1');
            aStkFee=await erc20_instance.aStkFee(std2);
            expect(aStkFee.toString()).to.equal('0'.toString());
        });

        it("11 - setDevFee to 25 to std2 (default)", async ()=> {
            await erc20_instance.setDevFee(std2, 0);
            devFee=await erc20_instance.readDevFee(std2);
            expect(devFee.toString()).to.equal('25'.toString());
        });

        it("12 - transfer between excluded and standars", async ()=> {
            try{
                await erc20_instance.transfer(std2, '9975'+weiToEth, { from: std1 });
                expect(1).to.equal(1);
            } catch(error){
                expect(error).to.equal("no debio entrar aca");
            }
        ...
        it("13 - Verify balances (error 0 ppm)", async ()=> {
            await checkBalance(owner,'19990000');
            await checkBalance(std1,'0');
            await checkBalance(std2,'9975');
            await checkBalance(dev,'25'); // acá no cobra pero ya tenía
        });
