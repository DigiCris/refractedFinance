...
contract('No Excluidas', accounts => {
    ...
    context("19- Tx entre una dir. excluida y otra no excluida, ambas con astkfee y devfee 0%", async () => {
        ...
        it("11 - transfer between standar and excluded", async ()=> {
            try{
        ...
        it("15 - transfer between excluded and standars", async ()=> {
            try{
                await erc20_instance.transfer(std2, '10000'+weiToEth, { from: std1 });
                expect(1).to.equal(1);
            } catch(error){
                expect(error).to.equal("no debio entrar aca");
            }
