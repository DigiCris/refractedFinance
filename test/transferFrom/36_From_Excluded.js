...
contract('No Excluidas', accounts => {
    ...
    context("36- TxFrom entre dos dir. excluidas, ambas con astkfee y devfee 0% ", async () => {
        ...
        it("17 - setAStkFee to 0 to std2", async ()=> {
            await erc20_instance.setAStkFee(std2, true, '0');
            aStkFee=await erc20_instance.aStkFee(std2);
            expect(aStkFee.toString()).to.equal('0'.toString());
        }); //_isExcluded

        it("18 - setDevFee to 0 to std2 (by -1)", async ()=> {
            await erc20_instance.setDevFee(std2, -1);
            devFee=await erc20_instance.readDevFee(std2);
            expect(devFee.toString()).to.equal('0'.toString());
        });

        it("19 - std2 excluded", async ()=> {
            const _isExcluded= await erc20_instance.isExcluded(std2);
            expect(_isExcluded.toString()).to.equal(true.toString());
        });

        it("20 - setAStkFee to 0 to std1", async ()=> {
            await erc20_instance.setAStkFee(std1, true, '0');
            aStkFee=await erc20_instance.aStkFee(std1);
            expect(aStkFee.toString()).to.equal('0'.toString());
        }); //_isExcluded

        it("21 - setDevFee to 0 to std1 (by -1)", async ()=> {
            await erc20_instance.setDevFee(std1, -1);
            devFee=await erc20_instance.readDevFee(std1);
            expect(devFee.toString()).to.equal('0'.toString());
        });

        it("22 - std1 excluded", async ()=> {
            const _isExcluded= await erc20_instance.isExcluded(std1);
            expect(_isExcluded.toString()).to.equal(true.toString());
        });
...
// excluded -> excluded (excluded)
        ...
        it("24 - owner approves 7970 to std1", async ()=> {//approve(address spender, uint256 value)
            try{//transferFrom(address sender, address recipient, uint256 amount) 
        ...
        it("25 - transferFrom between excluded and excluded ", async ()=> {//approve(address spender, uint256 value)
            try{//transferFrom(address sender, address recipient, uint256 amount) 
                await erc20_instance.transferFrom(std2, std1, '7970'+weiToEth, { from: std1 });
                expect(1).to.equal(1);
            } catch(error){
        ...
        it("26 - Allowance reduction to 0", async ()=> {
            _allowance = await erc20_instance.allowance(std2, std1);
        ...
        it("27 - Verify balances (error 0 ppm)", async ()=> {
            await checkBalance(owner,'19990053'+'985663836927012925');// 
