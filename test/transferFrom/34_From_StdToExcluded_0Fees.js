...
contract('No Excluidas', accounts => {
    ...
    context("34- TxFrom entre una dir. excluida y otra no excluida, ambas con astkfee y devfee 0% ", async () => {
        ...
        it("05 - setAStkFee to 0 to owner (-1)", async ()=> {
            await erc20_instance.setAStkFee(owner, false, '-1');
        ...
        it("15 - Verify balances (error 0 ppm)", async ()=> {
            await checkBalance(owner,'19990000');
            await checkBalance(std1,'10000');
            await checkBalance(dev,'0');
        });

        it("16 - setAStkFee to 0 to std2 (-1)", async ()=> {
            await erc20_instance.setAStkFee(std2, false, '-1');
            aStkFee=await erc20_instance.aStkFee(std2);
            expect(aStkFee.toString()).to.equal('0'.toString());
        });

        it("17 - setDevFee to 0 to std2", async ()=> {
        ...
        it("18 - transferFrom between excluded to Standar (must failed)", async ()=> {//approve(address spender, uint256 value)
            try{//transferFrom(address sender, address recipient, uint256 amount) 
                await erc20_instance.transferFrom(std1, std2, '10000'+weiToEth, { from: std2 });
                expect(1).to.equal("no debio entrar aca");
            } catch(error){
                expect(error.hijackedStack).to.include("Error: VM Exception while processing transaction: revert\n");
            }
        ...
        it("19 - owner approves 10000 to std1", async ()=> {//approve(address spender, uint256 value)
            try{//transferFrom(address sender, address recipient, uint256 amount) 
                await erc20_instance.approve(std2, '10000'+weiToEth, { from: std1 });
        ...
        it("21 - Allowance reduction to 0", async ()=> {
            _allowance = await erc20_instance.allowance(owner, std1);
            expect(_allowance.toString()).to.equal('0');
        });
        ...
        it("22 - Verify balances (error 0 ppm)", async ()=> {
            await checkBalance(owner,'19990000');
            await checkBalance(std1,'0');
            await checkBalance(std2,'10000');
            await checkBalance(dev,'0');
        });
