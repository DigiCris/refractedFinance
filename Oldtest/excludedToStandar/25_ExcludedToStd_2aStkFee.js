...
contract('No Excluidas', accounts => {
    ...
    context("25- Tx entre una dir. excluida y otra no excluida, ambas con astkfee positivo y ambas con devfee 0%.", async () => {
        ...
        it("05 - setAStkFee to 100 to owner", async ()=> { // no haría falta porque lo tengo por default
            await erc20_instance.setAStkFee(owner, false, '0'); 
        ...
        it("12 - Verify balances (error 50 ppm)", async ()=> {
            await checkBalance(owner,'19990049'+'987496874227816295');
            await checkBalance(std1,'9900');
        ...
        it("14 - setDevFee to 0 to std2 (by -1)", async ()=> {
            await erc20_instance.setDevFee(std2, -1);
        ...
        it("16 - Verify balances (error 0 ppm)", async ()=> {
            await checkBalance(owner,'19990053'+'985697585906459493');// aprox
            await checkBalance(std1,'1900'); // exacto
            await checkBalance(std2,'7992'+'001598476247319124'); // aprox (un poco mas tiene que tener)
            await checkBalance(dev,'0'); // exacto
        });
