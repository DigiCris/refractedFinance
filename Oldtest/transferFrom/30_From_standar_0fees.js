...
contract('No Excluidas', accounts => {
    ...
    context("30- TxFrom entre dos dir. no excluidas, ambas con astkfee y devfee 0% ", async () => {
        ...
        it("07 - transferFrom between standars", async ()=> {//approve(address spender, uint256 value)
        ...
        it("08 - owner approves 10000 to std1", async ()=> {//approve(address spender, uint256 value)
            try{//transferFrom(address sender, address recipient, uint256 amount) 
        ...
        it("09 - transferFrom between standars", async ()=> {//approve(address spender, uint256 value)
            try{//transferFrom(address sender, address recipient, uint256 amount) 
                await erc20_instance.transferFrom(owner, std1, '10000'+weiToEth, { from: std1 });
                expect(1).to.equal(1);
            } catch(error){
        ...
        it("10 - Allowance reduction to 0", async ()=> {
            _allowance = await erc20_instance.allowance(owner, std1);
